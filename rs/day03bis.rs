...
fn main() {
    let data: Vec<Vec<u32>> = include_str!("../data/day03_test.txt")
        .lines()
    ...
    let mut eps = 0;
    let mut dd: Vec<&Vec<u32>> = data.iter().filter(|_| true).collect();
    for i in 0..n {
        let col: Vec<u32> = dd.iter().map(|v| v[i as usize]).collect();
        let bit = match majority_elmt(&col) {
            Some(m) => m,
            None => 1,
        };
        eps += bit * 2_u32.pow(n - i - 1);
        dd = dd.iter().filter(|v| v[i as usize] == bit).map(|&v| v).collect();
    }
    println!("{:?}", eps);

    // println!("{:?}", data[0].len());
    // println!("{:?}", res);
    // println!("{:?}", majority_elmt(&firstcol));
}